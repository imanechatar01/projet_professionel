...
class Message {
    // Créer un message (client connecté)
    ...
    static async markAsRead(id) {
        const result = await pool.query(
            `UPDATE messages SET lu = true, updated_at = NOW() WHERE id = $1 RETURNING *`,
            [id]
        );
        return result.rows[0];
    }
    ...
    static async reply(id, reponse) {
        const result = await pool.query(
            `UPDATE messages SET reponse = $1, date_reponse = NOW(), updated_at = NOW() WHERE id = $2 RETURNING *`,
            [reponse, id]
        );
        return result.rows[0];
    }
    ...
    static async countUnread() {
        const result = await pool.query(`SELECT COUNT(*) FROM messages WHERE lu = false`);
